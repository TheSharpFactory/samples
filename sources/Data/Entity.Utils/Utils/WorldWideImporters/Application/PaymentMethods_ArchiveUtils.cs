...
{
    public static partial class PaymentMethods_ArchiveUtils
    {
    }
}
...
namespace TheSharpFactory.Data.Entity.Utils.WorldWideImporters.Application
{
    ...
    /// <summary>
    /// This class is a utility class for a TheSharpFactory.Data.Entity.WorldWideImporters.Application.PaymentMethods_Archive.
    /// </summary>
    public static partial class PaymentMethods_ArchiveUtils
    {
        /// <summary>
        ...
        public static bool HasChanges(PaymentMethods_Archive one, PaymentMethods_Archive two)
        {
        ...
        public static void Merge(PaymentMethods_Archive source, PaymentMethods_Archive target)
        {
        ...
        /// <returns>QueryFilters of PaymentMethods_ArchiveProperty</returns>
        public static QueryFilters<PaymentMethods_ArchiveProperty> GetChanges(PaymentMethods_Archive original, PaymentMethods_Archive changed)
        {
            // this method returns a list of changes.
            var changes = new QueryFilters<PaymentMethods_ArchiveProperty>(5);
            #region Detect Changes
            if(original.PaymentMethodID != changed.PaymentMethodID)
                changes.Add(QueryFilter.New(PaymentMethods_ArchiveProperty.PaymentMethodID, FilterConditions.Equals, changed.PaymentMethodID));
            if(string.CompareOrdinal(original.PaymentMethodName, changed.PaymentMethodName) != 0)
                changes.Add(QueryFilter.New(PaymentMethods_ArchiveProperty.PaymentMethodName, FilterConditions.Equals, changed.PaymentMethodName));
            if(original.LastEditedBy != changed.LastEditedBy)
                changes.Add(QueryFilter.New(PaymentMethods_ArchiveProperty.LastEditedBy, FilterConditions.Equals, changed.LastEditedBy));
            if(original.ValidFrom != changed.ValidFrom)
                changes.Add(QueryFilter.New(PaymentMethods_ArchiveProperty.ValidFrom, FilterConditions.Equals, changed.ValidFrom));
            if(original.ValidTo != changed.ValidTo)
                changes.Add(QueryFilter.New(PaymentMethods_ArchiveProperty.ValidTo, FilterConditions.Equals, changed.ValidTo));
            #endregion
