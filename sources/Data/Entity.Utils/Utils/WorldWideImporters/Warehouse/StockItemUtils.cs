...
{
    public static partial class StockItemUtils
    {
    }
}
...
namespace TheSharpFactory.Data.Entity.Utils.WorldWideImporters.Warehouse
{
    ...
    /// <summary>
    /// This class is a utility class for a TheSharpFactory.Data.Entity.WorldWideImporters.Warehouse.StockItem.
    /// </summary>
    public static partial class StockItemUtils
    {
        /// <summary>
        ...
        public static bool HasChanges(StockItem one, StockItem two)
        {
        ...
        public static void Merge(StockItem source, StockItem target)
        {
        ...
        /// <returns>QueryFilters of StockItemProperty</returns>
        public static QueryFilters<StockItemProperty> GetChanges(StockItem original, StockItem changed)
        {
            // this method returns a list of changes.
            var changes = new QueryFilters<StockItemProperty>(25);
            #region Detect Changes
            if(original.StockItemID != changed.StockItemID)
                changes.Add(QueryFilter.New(StockItemProperty.StockItemID, FilterConditions.Equals, changed.StockItemID));
            if(string.CompareOrdinal(original.StockItemName, changed.StockItemName) != 0)
                changes.Add(QueryFilter.New(StockItemProperty.StockItemName, FilterConditions.Equals, changed.StockItemName));
            if(original.SupplierID != changed.SupplierID)
                changes.Add(QueryFilter.New(StockItemProperty.SupplierID, FilterConditions.Equals, changed.SupplierID));
            if(original.ColorID != changed.ColorID)
                changes.Add(QueryFilter.New(StockItemProperty.ColorID, FilterConditions.Equals, changed.ColorID));
            if(original.UnitPackageID != changed.UnitPackageID)
                changes.Add(QueryFilter.New(StockItemProperty.UnitPackageID, FilterConditions.Equals, changed.UnitPackageID));
            if(original.OuterPackageID != changed.OuterPackageID)
                changes.Add(QueryFilter.New(StockItemProperty.OuterPackageID, FilterConditions.Equals, changed.OuterPackageID));
            if(string.CompareOrdinal(original.Brand, changed.Brand) != 0)
                changes.Add(QueryFilter.New(StockItemProperty.Brand, FilterConditions.Equals, changed.Brand));
            if(string.CompareOrdinal(original.Size, changed.Size) != 0)
                changes.Add(QueryFilter.New(StockItemProperty.Size, FilterConditions.Equals, changed.Size));
            if(original.LeadTimeDays != changed.LeadTimeDays)
                changes.Add(QueryFilter.New(StockItemProperty.LeadTimeDays, FilterConditions.Equals, changed.LeadTimeDays));
            if(original.QuantityPerOuter != changed.QuantityPerOuter)
                changes.Add(QueryFilter.New(StockItemProperty.QuantityPerOuter, FilterConditions.Equals, changed.QuantityPerOuter));
            if(original.IsChillerStock != changed.IsChillerStock)
                changes.Add(QueryFilter.New(StockItemProperty.IsChillerStock, FilterConditions.Equals, changed.IsChillerStock));
            if(string.CompareOrdinal(original.Barcode, changed.Barcode) != 0)
                changes.Add(QueryFilter.New(StockItemProperty.Barcode, FilterConditions.Equals, changed.Barcode));
            if(original.TaxRate != changed.TaxRate)
                changes.Add(QueryFilter.New(StockItemProperty.TaxRate, FilterConditions.Equals, changed.TaxRate));
            if(original.UnitPrice != changed.UnitPrice)
                changes.Add(QueryFilter.New(StockItemProperty.UnitPrice, FilterConditions.Equals, changed.UnitPrice));
            if(original.RecommendedRetailPrice != changed.RecommendedRetailPrice)
                changes.Add(QueryFilter.New(StockItemProperty.RecommendedRetailPrice, FilterConditions.Equals, changed.RecommendedRetailPrice));
            if(original.TypicalWeightPerUnit != changed.TypicalWeightPerUnit)
                changes.Add(QueryFilter.New(StockItemProperty.TypicalWeightPerUnit, FilterConditions.Equals, changed.TypicalWeightPerUnit));
            if(string.CompareOrdinal(original.MarketingComments, changed.MarketingComments) != 0)
                changes.Add(QueryFilter.New(StockItemProperty.MarketingComments, FilterConditions.Equals, changed.MarketingComments));
            if(string.CompareOrdinal(original.InternalComments, changed.InternalComments) != 0)
                changes.Add(QueryFilter.New(StockItemProperty.InternalComments, FilterConditions.Equals, changed.InternalComments));
            if(!Compare.ByteArrays(original.Photo, changed.Photo))
                changes.Add(QueryFilter.New(StockItemProperty.Photo, FilterConditions.Equals, changed.Photo));
            if(string.CompareOrdinal(original.CustomFields, changed.CustomFields) != 0)
                changes.Add(QueryFilter.New(StockItemProperty.CustomFields, FilterConditions.Equals, changed.CustomFields));
            if(string.CompareOrdinal(original.Tags, changed.Tags) != 0)
                changes.Add(QueryFilter.New(StockItemProperty.Tags, FilterConditions.Equals, changed.Tags));
            if(string.CompareOrdinal(original.SearchDetails, changed.SearchDetails) != 0)
                changes.Add(QueryFilter.New(StockItemProperty.SearchDetails, FilterConditions.Equals, changed.SearchDetails));
            if(original.LastEditedBy != changed.LastEditedBy)
                changes.Add(QueryFilter.New(StockItemProperty.LastEditedBy, FilterConditions.Equals, changed.LastEditedBy));
            if(original.ValidFrom != changed.ValidFrom)
                changes.Add(QueryFilter.New(StockItemProperty.ValidFrom, FilterConditions.Equals, changed.ValidFrom));
            if(original.ValidTo != changed.ValidTo)
                changes.Add(QueryFilter.New(StockItemProperty.ValidTo, FilterConditions.Equals, changed.ValidTo));
            #endregion
