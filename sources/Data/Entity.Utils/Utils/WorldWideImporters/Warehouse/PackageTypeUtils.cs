...
{
    public static partial class PackageTypeUtils
    {
    }
}
...
namespace TheSharpFactory.Data.Entity.Utils.WorldWideImporters.Warehouse
{
    ...
    /// <summary>
    /// This class is a utility class for a TheSharpFactory.Data.Entity.WorldWideImporters.Warehouse.PackageType.
    /// </summary>
    public static partial class PackageTypeUtils
    {
        /// <summary>
        ...
        public static bool HasChanges(PackageType one, PackageType two)
        {
        ...
        public static void Merge(PackageType source, PackageType target)
        {
        ...
        /// <returns>QueryFilters of PackageTypeProperty</returns>
        public static QueryFilters<PackageTypeProperty> GetChanges(PackageType original, PackageType changed)
        {
            // this method returns a list of changes.
            var changes = new QueryFilters<PackageTypeProperty>(5);
            #region Detect Changes
            if(original.PackageTypeID != changed.PackageTypeID)
                changes.Add(QueryFilter.New(PackageTypeProperty.PackageTypeID, FilterConditions.Equals, changed.PackageTypeID));
            if(string.CompareOrdinal(original.PackageTypeName, changed.PackageTypeName) != 0)
                changes.Add(QueryFilter.New(PackageTypeProperty.PackageTypeName, FilterConditions.Equals, changed.PackageTypeName));
            if(original.LastEditedBy != changed.LastEditedBy)
                changes.Add(QueryFilter.New(PackageTypeProperty.LastEditedBy, FilterConditions.Equals, changed.LastEditedBy));
            if(original.ValidFrom != changed.ValidFrom)
                changes.Add(QueryFilter.New(PackageTypeProperty.ValidFrom, FilterConditions.Equals, changed.ValidFrom));
            if(original.ValidTo != changed.ValidTo)
                changes.Add(QueryFilter.New(PackageTypeProperty.ValidTo, FilterConditions.Equals, changed.ValidTo));
            #endregion
